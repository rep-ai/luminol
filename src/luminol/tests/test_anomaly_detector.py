...
class TestAnomalyDetector(unittest.TestCase):
    ...
    def test_sign_test_algorithm(self):
        ...
        self.assertRaises(
            exceptions.RequiredParametersNotPassed,
            lambda: AnomalyDetector(
                self.s1, baseline_time_series=self.s2, algorithm_name="sign_test"
            ),
        )
        # test over specified
        algorithm_params = {
    ...
    def test_score_only(self):
        ...
        """
        detector = AnomalyDetector(
            self.s1, score_only=True, algorithm_name="derivative_detector"
        )
    ...
    def test_get_all_scores(self):
        ...
        self.assertEqual(
            len(self.detector1.get_all_scores()), len(self.detector1.time_series)
        )
    ...
    def test_algorithm_DefaultDetector(self):
        ...
        self.assertEqual(
            detector.get_all_scores().timestamps,
    ...
    def test_algorithm(self):
        ...
        self.assertRaises(
            ...
            lambda: AnomalyDetector(self.s1, algorithm_name="NotValidAlgorithm"),
        )
    ...
    def test_algorithm_params(self):
        ...
        self.assertRaises(
            ValueError,
            lambda: AnomalyDetector(
                self.s1, algorithm_name="exp_avg_detector", algorithm_params="0"
            ),
        )
        detector = AnomalyDetector(
            self.s1,
        ...
        self.assertNotEqual(
            self.detector1.get_all_scores().values, detector.get_all_scores().values
        )
    ...
    def test_anomaly_threshold(self):
        ...
        """
        detector = AnomalyDetector(
            self.s1, score_percent_threshold=0.1, algorithm_name="exp_avg_detector"
        )
        detector1 = AnomalyDetector(
            self.s1, score_percent_threshold=0.1, algorithm_name="derivative_detector"
        )
        self.assertNotEqual(detector1.get_anomalies(), detector.get_anomalies())

    def test_derivative_raw_score(self):
        """
        Test if derivative detector returns raw score instead of divinding by stdev
        """
        ts = {0: 2, 1: 3, 2: 150, 3: 5, 4: 1, 5: 2, 6: 2, 7: 2, 8: 0}
        detector = AnomalyDetector(
...
class CustomAlgo(AnomalyDetectorAlgorithm):
    ...
    def __init__(
        ...
        an anomaly
        """
        super(CustomAlgo, self).__init__(
            self.__class__.__name__, time_series, baseline_time_series
        )
    ...
    def _set_scores(self):
        ...
        for i, (timestamp, value) in enumerate(self.time_series.items()):
            ...
            if (
                self.percent_threshold_upper
                and diff_percent > 0
                and diff_percent > self.percent_threshold_upper
            ):
                anom_scores[timestamp] = diff_percent
            if (
                self.percent_threshold_lower
                and diff_percent < 0
                and diff_percent < self.percent_threshold_lower
            ):
