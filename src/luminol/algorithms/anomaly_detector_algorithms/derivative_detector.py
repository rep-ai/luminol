...
class DerivativeDetector(AnomalyDetectorAlgorithm):
    ...
    AM 2021: This method has been updated to use absolute values of the derivative,
    rather than a score relative to the data set's standard deviation. Human motion 
    cannot exceed certain speeds, so setting an upper limit for position data is reasonable.
    """
    ...
    def _compute_derivatives(self):
        ...
        for i, (timestamp, value) in enumerate(self.time_series_items):
            if i > 0:
                ...
                derivative = (value - pre_value) / \
                    td if td != 0 else value - pre_value
    ...
    def _set_scores(self):
        ...
        derivatives_ema = utils.compute_ema(
            self.smoothing_factor, self.derivatives)
        for i, (timestamp, value) in enumerate(self.time_series_items):
            anom_scores[timestamp] = abs(
                self.derivatives[i] - derivatives_ema[i])

        # AM 2021: edited to remove relative comaprison to standard dev.
